...
// new abbrs
function renderAbbrs(abbrGroup, newXScale, chosenXaxis) {
...
d3.csv("assets/data/healthData.csv", function(err, healthData) {
  ...
  var circlesGroup = chartGroup.selectAll("circle")
    ...
    var abbrGroupElem = chartGroup.append("g");


    // Add Text Labels
    var abbrGroup = abbrGroupElem.selectAll("text")
      .data(healthData)
      .enter()
      .append("text")
      .text(d => `${d.abbr}`)
      .attr("x", d => xLinearScale(d[chosenXAxis]-.25))
      .attr("y", d => yLinearScale(d.healthcare)+1.25)
      // scale issue for 2nd x axis abbrs not lining up
      //
  ...
  labelsGroup.selectAll("text")
    .on("click", function() {
      ...
      if (value !== chosenXAxis) {
        ...
        // updates abbrs with new x values
        abbrGroup = renderAbbrs(abbrGroup, xLinearScale, chosenXAxis);
